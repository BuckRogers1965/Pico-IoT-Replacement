...
blinkled(struct _task_entry_type * task, int led, int mesgid){
  ...
  case 1:
    ...
    AddTaskMilli (task, rate[led-15][0], &blinkled, 2, led);
    // see how we reschedule with mesgid set to 2
  ...
  case 2:
    ...
    AddTaskMilli (task, rate[led-15][1], &blinkled, 1, led);  
    // see how we reschedule with mesgid set to 1
...
void setup() {
  ...
  randomSeed(1);
  // Setup 8 timers to randomly blink leds on pins 15-22. 
  for (int i = 0; i<8; i++) {
    rate[i][0] = random(300, 1000);
    rate[i][1] = random(300, 1000);
    pinMode(i+15, OUTPUT); 
    AddTaskMilli (CreateTask(), rate[i][1], &blinkled, 1, i+15); 
  }
}
